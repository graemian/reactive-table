...
var setup = function () {
    ...
    this.autorun( () => {
        ...
        if (ready.get()) {
            ...
            const onFilterChange = this.data.settings.onFilterChange;

            if (onFilterChange)

                onFilterChange.call(context, getFilterCursor.call(context));
...
let getFilterCursor = function () {
    ...
    const cursor = this.collection.find(filterQuery);
    return cursor;
};
var getRowCount = function () {
    if (this.server) {
        ...
        const cursor = getFilterCursor.call(this);
        return cursor.count();
    }
};
